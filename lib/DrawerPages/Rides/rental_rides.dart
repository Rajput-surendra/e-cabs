...
class _RentalRidesState extends State<RentalRides> {
  ...
  getRides(type) async {
    try {
      ...
      Map response = await apiBase.postAPICall(Uri.parse(baseUrl1 + "Payment/rental_Bookint_by_user_id"), params);
      setState(() {
        loading = false;
        rideList.clear();
        _globalKey.clear();
                  ...
                  child: Container(
                    ...
                    child: Column(
                      children: [
                        ...
                        Divider(),
                        !rideList[index].bookingType!.contains("Point")?AnimatedTextKit(
                          animatedTexts: [
                            ColorizeAnimatedText(
                              rideList[index].bookingType
                                  .toString()
                                  .contains("Rental Booking")?"Rental Booking - ${rideList[index].start_time} - ${rideList[index].end_time}":"${getTranslated(context, "SCHEDULE")} - ${rideList[index].pickupDate} ${rideList[index].pickupTime}",
                        ...
                        Row(
                          ...
                          children: [
                            ...
                            InkWell(
                              onTap: () {
                                showBottom1(rideList[index].id,rideList[index].createdDate,index);
                              },
                              child: Container(
                                width: 30.w,
                                margin:
